...
static SocketHTTPServer_T
simple_http_server_create_core (SocketHTTPServer_Config *core_config)
{
...
Socket_simple_http_server_new_ex (const SocketSimple_HTTPServerConfig *config)
{
  ...
  SocketHTTPServer_T core_server = simple_http_server_create_core (&core_config);
  if (!core_server)
    {
#ifdef SOCKET_HAS_TLS
...
static bool
append_escape_char (char *dest, size_t *i_ptr, size_t dest_size, char escape)
{
  size_t i = *i_ptr;
  if (i >= dest_size - 2) return false;
  dest[i++] = '\\';
  dest[i++] = escape;
  *i_ptr = i;
  return true;
...
static bool
append_unicode_escape (char *dest, size_t *i_ptr, size_t dest_size, unsigned char c)
{
  size_t i = *i_ptr;
  int written = snprintf (&dest[i], dest_size - i, "\\u%04x", c);
  if (written <= 0 || (size_t)written >= dest_size - i) return false;
  *i_ptr = i + written;
  return true;
}
  ...
  while (*src && i < dest_size - 1)
    {
      ...
      switch (c)
        {
        case '\"':
          appended = append_escape_char (dest, &i, dest_size, '\"');
          break;
        case '\\':
          appended = append_escape_char (dest, &i, dest_size, '\\');
          break;
        case '\b':
          appended = append_escape_char (dest, &i, dest_size, 'b');
          break;
        case '\f':
          appended = append_escape_char (dest, &i, dest_size, 'f');
          break;
        case '\n':
          appended = append_escape_char (dest, &i, dest_size, 'n');
          break;
        case '\r':
          appended = append_escape_char (dest, &i, dest_size, 'r');
          break;
        case '\t':
          appended = append_escape_char (dest, &i, dest_size, 't');
          break;
        default:
          if (c < 0x20)
            {
              appended = append_unicode_escape (dest, &i, dest_size, c);
            }
          else
            {
              dest[i++] = c;
