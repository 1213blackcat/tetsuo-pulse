...
#endif
static SocketProxy_Result
proxy_validate_proxy_target (const SocketProxy_Config *proxy,
                             const char *target_host,
                             int target_port)
{
...
static int
proxy_make_nonblocking (int fd, int *was_nonblocking)
{
  int flags = fcntl (fd, F_GETFL);
  if (flags < 0)
    return -1;
  *was_nonblocking = (flags & O_NONBLOCK) != 0;
  if (!*was_nonblocking)
...
static void
proxy_restore_blocking (int fd, int was_nonblocking)
{
  if (was_nonblocking)
    return;
  int flags = fcntl (fd, F_GETFL);
                    ...
                    Arena_T arena /* optional, NULL ok */)
{
  ...
  assert (target_host != NULL);
  assert (target_port > 0 && target_port <= SOCKET_MAX_PORT);
  result = proxy_validate_proxy_target (proxy, target_host, target_port);
  if (result != PROXY_OK)
    return result;
  proxy_tunnel_init_context (conn, socket, proxy, target_host, target_port, arena);
  fd = Socket_fd (socket);
  if (proxy_make_nonblocking (fd, &was_nonblocking) < 0)
    {
      socketproxy_set_error (conn, PROXY_ERROR, "Failed to set non-blocking");
      result = PROXY_ERROR;
      goto cleanup;
    }
#if SOCKET_HAS_TLS
  ...
  result = conn->result;
cleanup:
  proxy_restore_blocking (fd, was_nonblocking);
  return result;
}
static int
