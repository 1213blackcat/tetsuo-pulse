...
/* Helper: Deflate the input */
static int
deflate_input (SocketWS_T ws, const unsigned char *input, size_t input_len,
               unsigned char *buf, size_t buf_size, size_t *written, size_t *consumed)
{
  SocketDeflate_Result res = SocketDeflate_Deflater_deflate (ws->compression.deflater,
                                                             input,
                                                             input_len,
                                                             consumed,
                                                             buf,
...
/* Helper: Finish deflate with final block */
static int
finish_deflate (SocketWS_T ws, unsigned char *buf, size_t buf_size, size_t written)
{
  size_t finish_written;
  SocketDeflate_Result res = SocketDeflate_Deflater_finish (ws->compression.deflater,
                                                            buf + written,
                                                            buf_size - written,
                                                            &finish_written);
  if (res != DEFLATE_OK)
  ...
  return (int)finish_written;
}
/* Helper: Sync flush deflate */
static int
sync_flush_deflate (SocketWS_T ws, unsigned char *buf, size_t buf_size, size_t written)
{
  size_t flush_written;
  SocketDeflate_Result res = SocketDeflate_Deflater_sync_flush (ws->compression.deflater,
                                                                buf + written,
                                                                buf_size - written,
                                                                &flush_written);
  if (res != DEFLATE_OK)
                     ...
                     size_t *output_len)
{
  ...
  if (deflate_input (ws, input, input_len, buf, buf_size, &written, &consumed) < 0)
    return -1;
  ...
  if (should_reset_context (ws, 1 /* deflate */))
    {
      ...
      flush_written = finish_deflate (ws, buf, buf_size, written);
      if (flush_written < 0)
        return -1;
  ...
  else
    {
      ...
      flush_written = sync_flush_deflate (ws, buf, buf_size, written);
      if (flush_written < 0)
        return -1;
    }
  written += (size_t)flush_written;
...
/* Helper: Inflate loop body */
static int
inflate_loop_body (SocketWS_T ws, const unsigned char *input_ptr, size_t input_remaining,
                   unsigned char *buf, size_t buf_size, size_t *total_written, size_t *consumed,
                   size_t *written)
{
  SocketDeflate_Result res = SocketDeflate_Inflater_inflate (ws->compression.inflater,
                                                              input_ptr,
                                                              input_remaining,
                                                              consumed,
                                                              buf + *total_written,
  ...
  if (res == DEFLATE_INCOMPLETE && *consumed == 0 && *written == 0)
    {
      ws_set_error (ws, WS_ERROR_COMPRESSION, "Inflate incomplete: no progress");
      return -1;
    }
  if (res == DEFLATE_ERROR_BOMB)
    {
      ws_set_error (ws, WS_ERROR_MESSAGE_TOO_LARGE, "Decompression bomb detected");
      return -1;
    }
  return (int)res;
}
                       ...
                       size_t *output_len)
{
  ...
  while (input_remaining > 0)
    {
      int inflate_res = inflate_loop_body (ws, input_ptr, input_remaining, buf, buf_size,
                                           &total_written, &consumed, &written);
      if (inflate_res < 0)
        return -1;
      total_written += written;
      input_ptr += consumed;
      input_remaining -= consumed;
      if ((SocketDeflate_Result)inflate_res == DEFLATE_OUTPUT_FULL)
        {
          if (ws_inflate_grow_buffer (ws, &buf, &buf_size, total_written) != 0)
            return -1;
          continue;
        }
      if ((SocketDeflate_Result)inflate_res == DEFLATE_OK)
        break;
    }
...
/* Helper: Allocate compress buffer */
static unsigned char *
alloc_compress_buffer (SocketWS_T ws, size_t buf_size)
{
  if (!SocketSecurity_check_size (buf_size))
    {
      ws_set_error (ws,
  ...
  unsigned char *buf = ALLOC (ws->arena, buf_size);
  if (!buf)
    {
      ws_set_error (
...
/* Helper: Set up zlib compress stream */
static int
setup_compress_stream (SocketWS_T ws, z_stream *strm, const unsigned char *input, size_t input_len,
                     ...
                     size_t *output_len)
{
  ...
  buf = alloc_compress_buffer (ws, buf_size);
  if (!buf)
    return -1;
  if (setup_compress_stream (ws, strm, input, input_len, buf, buf_size) < 0)
    return -1;
...
/* Helper: Allocate decompress buffer */
static unsigned char *
alloc_decompress_buffer (SocketWS_T ws, size_t buf_size)
{
  if (!SocketSecurity_check_size (buf_size))
    {
      ws_set_error (ws,
...
/* Helper: Set up zlib decompress stream */
static int
setup_decompress_stream (SocketWS_T ws, z_stream *strm, const unsigned char *input_with_trailer,
                         size_t input_with_trailer_len, unsigned char *buf, size_t buf_size)
{
                       ...
                       size_t *output_len)
{
  ...
  buf = alloc_decompress_buffer (ws, buf_size);
  if (!buf)
    return -1;
  if (setup_decompress_stream (ws, strm, input_with_trailer, input_with_trailer_len, buf, buf_size) < 0)
    return -1;
  /* Decompress */
