...
return -1;
StreamSegment_T *seg
= Arena_alloc (t->arena, sizeof (*seg), FILE, LINE);
memset (seg, 0, sizeof (*seg));
seg->offset = offset;
seg->length = length;
seg->data = Arena_alloc (t->arena, (size_t)length, FILE, LINE);
...
&& t->loss[QUIC_PN_SPACE_APPLICATION]->sent_count= QUIC_LOSS_MAX_SENT_PACKETS)
return -1;
...
static int derive_and_set_keys(SocketQUICTransport_T t, SocketQUICCryptoLevel level) {
...
read_secret,
&secret_len)
!= QUIC_TLS_OK) return -1;
SocketQUIC_AEAD aead = aead_from_secret_len (secret_len);
switch(level) {
case QUIC_CRYPTO_LEVEL_0RTT:
SocketQUICCrypto_derive_packet_keys (write_secret, secret_len, aead, &t->zero_rtt_send_keys);
t->zero_rtt_keys_valid = 1;
break;
case QUIC_CRYPTO_LEVEL_HANDSHAKE:
SocketQUICCrypto_derive_packet_keys (write_secret, secret_len, aead, &t->handshake_send_keys);
SocketQUICCrypto_derive_packet_keys (read_secret, secret_len, aead, &t->handshake_read_keys);
SocketQUICReceive_set_handshake_keys (&t->recv_ctx, &t->handshake_read_keys);
t->handshake_keys_valid = 1;
break;
case QUIC_CRYPTO_LEVEL_APPLICATION:
SocketQUICCrypto_derive_packet_keys (write_secret, secret_len, aead, &t->app_send_keys);
SocketQUICKeyUpdate_set_initial_keys (&t->key_update, write_secret, read_secret, secret_len, aead);
...
for (size_t i = 0; i < sizeof (write_secret); i++) vw[i] = 0;
for (size_t i = 0; i < sizeof (read_secret); i++) vr[i] = 0;
return 0;
}
...
{
derive_and_set_keys(t, QUIC_CRYPTO_LEVEL_0RTT);
}
if (!t->handshake_keys_valid
&& SocketQUICHandshake_has_keys (t->handshake,
QUIC_CRYPTO_LEVEL_HANDSHAKE))
{
derive_and_set_keys(t, QUIC_CRYPTO_LEVEL_HANDSHAKE);
}
if (!t->app_keys_valid
&& SocketQUICHandshake_has_keys (t->handshake,
QUIC_CRYPTO_LEVEL_APPLICATION))
{
derive_and_set_keys(t, QUIC_CRYPTO_LEVEL_APPLICATION);
}
}
void
...
= Arena_alloc (arena, sizeof (*t), FILE, LINE);
memset (t, 0, sizeof (*t));
t->arena = arena;
if (config)
t->config = config;
...
t->send_buf
= Arena_alloc (arena, TRANSPORT_SEND_BUF_SIZE, FILE, LINE);
t->recv_buf
= Arena_alloc (arena, TRANSPORT_RECV_BUF_SIZE, FILE, LINE);
...
static int
generate_connection_ids (SocketQUICTransport_T t)
{
...
static int
create_connection_and_handshake (SocketQUICTransport_T t)
{
...
static int
init_initial_keys (SocketQUICTransport_T t)
{
...
static int
init_ack_loss (SocketQUICTransport_T t)
{
for (int space = 0; space < QUIC_PN_SPACE_COUNT; space++)
{
int is_initial = (space != QUIC_PN_SPACE_APPLICATION);
int ack_immediate = is_initial ? 1 : 0;
uint64_t max_delay = is_initial ? 0 : QUIC_ACK_DEFAULT_MAX_DELAY_US;
t->ack[space] = SocketQUICAck_new (t->arena, ack_immediate, max_delay);
if (!t->ack[space])
return -1;
t->loss[space] = SocketQUICLoss_new (t->arena, ack_immediate, max_delay);
if (!t->loss[space])
return -1;
}
return 0;
...
if (!setup_ok)
return -1;
if (generate_connection_ids (t) < 0)
goto fail;
if (create_connection_and_handshake (t) < 0)
goto fail;
/ Phase B: TLS and 0-RTT setup /
if (complete_handshake_setup (t) < 0)
goto fail;
/ Phase C: Initial packet /
if (init_initial_keys (t) < 0)
goto fail;
/ Init ACK states /
if (init_ack_loss (t) < 0)
goto fail;
...
static void
update_flow_from_peer_params (SocketQUICTransport_T t)
{
...
static int
handle_0rtt_rejection (SocketQUICTransport_T t)
{
