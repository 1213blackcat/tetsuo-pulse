...
= Arena_alloc (arena, sizeof (*seg), FILE, LINE);
if (!seg)
{
...
seg->offset = offset;
seg->length = length;
seg->data = Arena_alloc (arena, (size_t)length, FILE, LINE);
if (!seg->data)
{
return QUIC_HANDSHAKE_ERROR_MEMORY;
}
...
= Arena_alloc (arena, sizeof (*hs), FILE, LINE);
if (!hs)
{
return NULL;
}
...
arena, QUIC_HANDSHAKE_CRYPTO_BUFFER_SIZE, FILE, LINE);
}
...
static void
free_tls_resources (SocketQUICHandshake_T hs)
{
#ifdef HAVE_OPENSSL
if (hs->tls_ssl)
{
...
static void
secure_clear_keys (SocketQUICHandshake_T hs)
{
for (int i = 0; i < QUIC_CRYPTO_LEVEL_COUNT; i++)
{
if (hs->keys[i])
...
static void
secure_clear_0rtt_data (SocketQUICHandshake_T hs)
{
...
free_tls_resources (hs);
secure_clear_keys (hs);
secure_clear_0rtt_data (hs);
*handshake = NULL;
}
...
Validate connection and handshake context


Derive Initial secrets from DCID (RFC 9001 Section 5.2)


Generate ClientHello via TLS (RFC 9001 Section 4)


Prepare CRYPTO frame (RFC 9000 Section 19.6)


Protect and send packet (RFC 9001 Section 5.4, RFC 9000 Section 17.2.2)
...
static SocketQUICHandshake_Result
setup_local_transport_params (SocketQUICHandshake_T handshake)
{
...
static SocketQUICHandshake_Result
advance_tls_state (SocketQUICHandshake_T handshake)
{
...
static SocketQUICHandshake_Result
retrieve_peer_params (SocketQUICHandshake_T handshake)
{
...
static SocketQUICHandshake_Result
process_0rtt_status (SocketQUICHandshake_T handshake)
{
/
...
/
SocketQUICHandshake_Result res = setup_local_transport_params (handshake);
if (res != QUIC_HANDSHAKE_OK)
return res;
/ Advance TLS state machine /
res = advance_tls_state (handshake);
if (res != QUIC_HANDSHAKE_OK)
return res;
/ After handshake complete, retrieve and decode peer params (RFC 9001 §8.2)
/
res = retrieve_peer_params (handshake);
if (res != QUIC_HANDSHAKE_OK)
return res;
res = process_0rtt_status (handshake);
return res;
}
int
...
= Arena_alloc (handshake->arena, ticket_len, FILE, LINE);
